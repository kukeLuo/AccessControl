...
public class Chinese2Pinyin {
    ...
    public static String changeToTonePinYin(String pinYinStr) {
        ...
        try {
            tempStr = PinyinHelper.convertToPinyinString(pinYinStr, "", PinyinFormat.WITHOUT_TONE).toLowerCase();
        } catch (Exception e) {
            e.printStackTrace();
        }
     ...
     * @return 拼音字符串
     */
    public static String changeToGetCapLetters(String pinYinStr) {
        String tempStr = "";

        try {
            tempStr = PinyinHelper.getShortPinyin(pinYinStr).toLowerCase();
        } catch (Exception e) {
            e.printStackTrace();
        }
